#!/usr/bin/env python3
"""Launch LinkedIn jobs MCP server using the repo venv when available."""
...
def main() -> int:
    ...
    if venv_py is not None and Path(sys.executable).resolve() != venv_py.resolve():
        ...
        os.execv(
            str(venv_py),
            [str(venv_py), "-m", "agentzero.linkedin_mcp_server", *sys.argv[1:]],
        )
    if str(repo) not in sys.path:
        sys.path.insert(0, str(repo))
    from agentzero.linkedin_mcp_server import main as mcp_main
