...
LEAD_SESSION_WORKFLOW: list[dict[str, str]] = [
    ...
    },
    {
        ...
        "prompt_user": (
            ...
            "New roles land as lead in SQLite — not on the sheet yet."
        ),
    },
    {
        "step": "4",
    ...
    },
    {
        ...
        "action": "commit_leads",
        "prompt_user": (
            "After user picks job_ids, commit approved leads to the Google Sheet. "
            "Warn that sync rewrites the whole worksheet (all exportable DB rows). "
            "Report actual approved count and sheet row count."
        ),
    },
]
...
- Ask before every scrape and before every sheet commit.
- reject_leads keeps rows in SQLite for dedupe but excludes them from sheet export.
- commit_leads rewrites the entire worksheet with all exportable DB rows (not just the ids passed).
