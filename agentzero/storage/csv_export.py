...
def job_to_row(job: JobPosting, *, today: date | None = None) -> dict[str, object]:
    return {
        ...
        "easy_apply_url": job.easy_apply_url or "",
        "easy_apply": job.easy_apply if job.easy_apply is not None else "",
        "careers_url": job.careers_url or "",
        "company_website": job.company_website or "",
        "is_public_company": job.is_public_company if job.is_public_company is not None else "",
        "stock_ticker": job.stock_ticker or "",
        "linkedin_connections_note": job.linkedin_connections_note or "",
